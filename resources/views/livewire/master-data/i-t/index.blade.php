<div class="p-4 space-y-4">
    ...
    <div class="flex justify-between items-center">
        <div class="relative w-full max-w-md">
            <input type="text" wire:model.live="search" placeholder="Cari nama/email..." autocomplete="search"
        ...
        <table class="w-full text-sm text-left">
            ...
            <tbody class="text-gray-800 dark:text-gray-200">
                ...
                <tr class="border-t border-gray-200 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800 transition">
                    ...
                    <td class="px-4 py-2">
                        @if($row->image)
                        <img src="{{ Storage::url($row->image) }}" class="w-10 h-10 rounded-full object-cover mx-auto" alt="img" />
                        @else
                    ...
                    <td class="px-4 py-2 flex flex-wrap gap-1 text-xs">
                        @can('datait.update')
                        <button wire:click="edit({{ $row->id }})" class="inline-flex items-center gap-1 px-2 py-1 rounded bg-blue-900 text-blue-200 hover:bg-blue-800 hover:ring-2 hover:ring-blue-400/60 transition duration-200" title="Edit">
                            <!-- Icon Edit -->
                        ...
                        @can('datait.delete')
                        <button onclick="confirmDelete({{ $row->id }})" title="Delete" class="inline-flex items-center gap-1 px-2 py-1 rounded bg-red-900 text-red-200 hover:bg-red-800 hover:ring-2 hover:ring-red-400/60 transition duration-200">
                            <!-- Icon Delete -->
                            <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24"
                                stroke="currentColor" stroke-width="2">
                        ...
                        @can('datait.read')
                        <button wire:click="view({{ $row->id }})" title="Lihat Detail" class="inline-flex items-center gap-1 px-2 py-1 rounded bg-gray-800 text-gray-100 hover:bg-gray-700 hover:ring-2 hover:ring-gray-400/60 transition duration-200">
                            <!-- Icon View -->
                            <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24"
                                stroke="currentColor" stroke-width="2">
                                <path stroke-linecap="round" stroke-linejoin="round"
                ...
                @empty
                <tr>
                    <td colspan="9" class="px-4 py-4 text-center text-gray-500 dark:text-gray-400">Tidak ada data</td>
                </tr>
                @endforelse
            </tbody>
        </table>
    ...
    @if($showModal)
    @include('livewire.master-data.i-t.partials.modal-form')
    @endif

    @if($showDetailModal)
    @include('livewire.master-data.i-t.partials.modal-detail')
    @endif
